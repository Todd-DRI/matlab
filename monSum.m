function [tS_mon, data_mon] = monSum(tS_in, data, t_start,t_end, plot_flag)
% creates monthly sum through final day of that month and a matrix 'data' over a period from t_start to t_end
% V1.0, 2/13/20, TGC
...
%% remove any excess time periods
ind = tS_mon < t_start | tS_mon > t_end;
tS_mon(ind) = [];
data_mon(ind,:) = [];
...
if plot_flag == 1    
    ...
    for i = 1:col
        ...
        plot(tS_mon, cumsum(data_mon(:,i),'omitnan'), 'ob','MarkerFaceColor','w')
        ylabel('Cum. sum')
        hold off
        xlim([t_start t_end])
        %ylim([0 0.6])
