function [I]=my_GEOTIFF_READ(varargin)
% SINGLE BAND (TGC) GEOTIFF_READ: read geotiff using imread and assign map info from infinfo.
% I couldn't get the original function to read an entire file, and
% importdata seems easier! 
%% Original function
...
%name = sprintf('%s/d2r.tif', polaris_dirname)% ++++++++++++++++++++++++++++++REMOVE
...
% fid = fopen(name,'r','ieee-le');
% 
% fid = fopen(sprintf('%s/d2r.tif', polaris_dirname),'r','ieee-le'); % %remove
% 
% fseek(fid,offset1,'bof'); 
% 
% tmp=zeros(sub(4)-sub(3)+1, sub(2)-sub(1)+1,info.bands,format);
...
% 
% fclose(fid);
%% Simpler read version
tmp = importdata(name);

%% orginal function's final structure 
I.z=tmp;
I.info = info;
